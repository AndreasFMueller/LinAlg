...
\def\w{8}
\node at ({-\w/2},0) {\includeagraphics[width=8cm]{tubes.jpg}};
\begin{scope}
\clip (0,0) rectangle (\w,{\w/2});
\node at ({\w/2},{\w/2-0.6}) {\includeagraphics[width=8cm]{intersect.jpg}};
\end{scope}
\begin{scope}
\clip (0,{-\w/2}) rectangle (\w,0);
\node at ({\w/2},-2.2) {\includeagraphics[width=8cm]{inside.jpg}};
\end{scope}
\end{tikzpicture}
\end{center}
\caption{3D-Darstellung der Röhren aus Aufgabe~\ref{50000014}.
Rechts die Berührungsstelle der beiden Rohre und rechts unten
der Teil des brauen Rohres, welcher im inneren des hellen Rohres
sichtbar wird.
...
% \item
% Wir müssen herausfinden, ob sich die Geraden $AB$ und $EF$ schneiden.
...
% Lösung existiert, die Geraden schneiden sich also.
% %rref(A)
% %ans =
% %
...
% %\alpha&=95.553^\circ
% %\end{align*}
% %Natürlich ist auch $\alpha=84.447^\circ$ eine korrekte Antwort.
% \end{teilaufgaben}
\end{loesung}
