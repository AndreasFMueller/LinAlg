...
\begin{tikzpicture}[>=latex, scale=1,
   ...
   connection/.style={draw=black,thick,->},
	...
	\node[decision,inner sep=1pt](s1) at(0,-2*\step){\textcolor{greenT}{\parbox{3cm}{\centering Hat es frei wÃ¤hl- bare Variabeln?}}};

	\node[processStep](s2) at(\shift,-1*\step){Keine LÃ¶sung};
	\node[processStep](s3) at(\shift,-2*\step){Eine LÃ¶sung};
	\node[processStep](s4) at(0,-2.9*\step){$\infty$-viele LÃ¶sungen};
