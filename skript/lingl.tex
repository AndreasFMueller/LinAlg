...
zusammenzustellen.
Dabei interessiert uns nicht nur die L"osung
...
\end{compactenum}
Genau genommen sind dies nicht drei gleichberechtigte F"alle,
sondern
...
Vollst"andig ausgeschrieben lauten die Gleichungssysteme
\begin{align*}
...
F"ugt man jetzt eine zweite Gleichung hinzu, erh"alt man
...
L"osung haben, k"onnen wir bereits herausfinden, wie viele
...
\end{linsys}
\end{equation}
Die zugeh"orige Folge von Tableaus ist
\begin{align*}
...
zun"achst den vordersten nicht verschwindenden Koeffizienten, das sogenannte
Pivot-Element zu $1$.
\index{Pivot-Element}
...
F"ur die dritte Spalte beispielsweise bedeutet dies bildlich:
\begin{gather*}
\begin{tabular}{|>{$}c<{$}>{$}c<{$}>{$}c<{$}>{$}c<{$}>{$}c<{$}|>{$}c<{$}|}
\hline
1&*&*&\dots&0&*\\
...
\end{linsys}.
\]
Man beachte, dass das Koeffizientenschema gegen"uber dem
...
$\operatorname{Rang}A$.
\end{satz}
Aus dem Rang l"asst sich also auch ermitteln, wie viele frei w"ahlbare
...
Wir "uberlegen uns zun"achst, wie das Schlusstableau des modifizierten
...
eine "ublicherweise \texttt{rref} genannte Funktion zur Verf"ugung, welche die 
...
fehlt uns jetzt noch eine Verkn"upfung zwischen Matrizen und
Vektoren.
